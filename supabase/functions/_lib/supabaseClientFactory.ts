...
export function getAdminClient(): SupabaseClient {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error("NEXT_PUBLIC_SUPABASE_URL ou SUPABASE_SERVICE_ROLE_KEY não definidos no ambiente da função!");
  }
...
export function getRLSClient(token: string): SupabaseClient {
  const supabaseUrl = Deno.env.get("NEXT_PUBLIC_SUPABASE_URL");
  if (!supabaseUrl) {
    throw new Error("NEXT_PUBLIC_SUPABASE_URL não definido no ambiente da função!");
  }
